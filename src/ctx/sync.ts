/**
 * Main sync() function — the core of @zynd/ctx.
 *
 * Orchestrates: connector fan-out → distill → ingest text → declare facts
 * → enrich AgentDNS card. Returns a SyncResult summarizing what happened.
 */
...
export async function sync(opts: SyncOpts): Promise<SyncResult> {
  ...
  if (result.textBlob.length >= 40) {
    try {
      ...
      console.error(`[zynd/ctx] ingest failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  if (result.findabilityFacts.length > 0) {
    try {
      ...
      console.error(`[zynd/ctx] declare-batch failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  if (agentId && keypair && registryUrl && result.cardEnrichment.capabilities.length > 0) {
    try {
      ...
      console.error(`[zynd/ctx] AgentDNS card update failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
