...
export class LinkedInConnector implements IMemoryConnector {
  ...
  async health(): Promise<ConnectorHealth> {
    if (!this.liAt) return { connected: false, error: "not authenticated — run: zynd ctx linkedin-auth" };
  ...
  async distill(): Promise<DistillResult> {
    ...
    try {
      ...
      if (msg.includes("session_expired") || msg.includes("li_at")) {
        throw new Error("LinkedIn session expired — run: zynd ctx linkedin-auth");
      }
      throw new Error(`LinkedIn sync failed: ${msg}`, { cause: err });
    }

    const parts: string[] = [];
    if (result.profile_text) parts.push(`My LinkedIn profile:\n${result.profile_text}`);
    if (result.connections.length > 0) {
      const connText = result.connections
        .map(c => [
          c.name ? `Name: ${c.name}` : null,
          c.headline ? `Role: ${c.headline}` : null,
